...
export function isOrphanListMarkerLine(stripped: string): boolean {
    if (!stripped) return false;
    if (/^[-*+•]$/.test(stripped)) return true;
    if (/^\d{1,9}\.$/.test(stripped)) return true;
    if (/^\d{1,9}[.)]$/.test(stripped)) return true;
    if (/^[-*+]\s*\[\s*[xX ]?\s*\]$/.test(stripped)) return true;
    if (/^\.+$/.test(stripped)) return true;
    return false;
}
